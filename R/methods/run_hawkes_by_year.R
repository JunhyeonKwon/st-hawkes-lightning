...
run_year_analysis <- function(year, data_dir, output_dir, config) {
  # Run all windows for one year and save a single RDS file for that year.
  message(sprintf("[%d] Loading data.", year))
  lightning_year <- readRDS(file.path(data_dir, sprintf("Lightningevent_%d.RDS", year)))
  ...
  for (batch_id in seq_along(batch_plan)) {
    ...
    message(
      sprintf(
        "[%d] Batch %d/%d (%s): %d windows on %d cores.",
        year,
        batch_id,
        length(batch_plan),
        batch$label,
  ...
  output_path <- file.path(
    output_dir,
    sprintf(
      "nonpar_results%d_st_inhomo_bgr_sden_%s_v3.rds",
      year,
      config$sden_est_type
    )
  )

  saveRDS(result_total, output_path)
  message(sprintf("[%d] Saved results to %s", year, output_path))
